...
public class IngresoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        LocalDate fechaSeleccionada = (fechaParam != null && !fechaParam.isBlank())
                ? LocalDate.parse(fechaParam)
                : LocalDate.now();

        List<Ingreso> lista = fechaSeleccionada.isEqual(LocalDate.now())
                ? ingresoService.listarIngresosDelDia()
                : ingresoService.listarPorFecha(fechaSeleccionada);

        request.setAttribute("listaIngresos", lista);
        request.setAttribute("fechaSeleccionada", fechaSeleccionada.toString());
